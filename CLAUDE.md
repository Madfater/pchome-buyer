...
- **`session.py`** — `login_flow(wait_for_user)` (headed browser, caller decides how to block: CLI uses `input()`, web uses `Event.wait()`), auth-state save/existence check (the runner passes `AUTH_STATE_FILE` as a path to `new_context(storage_state=...)`), `check_session()` (loads the cart page and detects redirect to `ecvip.pchome.com.tw/login/...`; snapup/cart modify work without login — login is only enforced at checkout, so this runs before monitoring starts).
