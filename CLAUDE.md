...
uv run pytest                           # 改了 pchome/ 任何 Python 必跑（覆蓋 core/services/api 大部分模組，2 秒內跑完）
...
**每個 session 的底線**（詳細判準在 20-judgment-rubrics.md）：改 Python 跑 pyright＋`uv run pytest`、改前端跑 lint+build（動到 `frontend/src` 另跑 `npm run test`）；行為改動要被實際執行過才算完成；驗收派 fresh-context agent，不自驗；教訓寫回對應檔案的「教訓紀錄」段。
