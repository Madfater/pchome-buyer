...
> **最後校準：2026-07-06**（設定視窗：CVC/AUTO_PAY 與搶購時機/進階調校參數移入 MongoDB `settings` collection，`get_cvc()`/`is_auto_pay()`/`.env` 的 CVC/AUTO_PAY 已移除；尚未 commit）。距今超過 30 天或經歷大型 refactor 後，引用前應抽查校準。
...
- **`runner.py`** — `JobConfig` 除了 `product_ids`/`sale_ts`/`interval`/`lead`/`headless`，還有 `fast_poll_window_secs`/`slow_poll_factor`/`resync_secs`/`max_retries`/`retry_delay_secs`/`cvc`/`auto_pay`（皆有出廠預設值，供 CLI 沿用）。`run_snapup_job(JobConfig, reporter, membership=, checkout_lock=, cancel=, hold=)`：lead 睡眠 → 開瀏覽器 → session 檢查 → 監控（帶入 `cfg` 的輪詢調校參數）→ freeze membership → 加購重試（帶入 `cfg` 的重試參數）→ 結帳（帶入 `cfg.cvc`/`cfg.auto_pay`）→ `hold(result)`（保持瀏覽器開啟；以 pending `JobResult` 呼叫，讓 web 層在瀏覽器還開著時就持久化結帳紀錄）。回傳 `JobResult(status, success_ids, cart_results, checkout)`。CLI 不傳 membership（由 `cfg.product_ids` 建靜態的），且 CLI 的 `cvc`/`auto_pay`/進階調校欄位目前固定用 `JobConfig` 出廠預設值（`cli.py` 這次的設定視窗改動刻意不接 `SettingsStore`，見 [50-letter.md](50-letter.md) 的 CLI 移除計畫）。
